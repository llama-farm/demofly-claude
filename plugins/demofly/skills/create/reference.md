...
**Philosophy: Narrate First, Record Second.** Narration drives timing — not the
other way around. Write the story, generate audio, then record video that serves
the narration. Intelligent assembly reconciles any timing drift.

```
context.md → proposal.md → narration.md → audio/ + timestamps.json
                (user approves)                |
                                               v
                                    ...
                                    (Whisper)         (LLM, auto-applied)   (ffmpeg retiming)
```
...
### narration.md Format — Pure Storytelling

narration.md is the storytelling source of truth. It contains **no Playwright
actions or selectors** — only the narrative. TTS reads this file to generate
audio; the action map (script.md) is derived from it afterward.

```markdown
# Narration: [Demo Name]

**Product:** [Product name]
**Target duration:** [e.g. 90s]
**Persona:** [Polished Keynote | Engineering Standup | Hype Marketing]
**Scenes:** [count]

## Beat Map

| Scene | Emotional Beat      | UI Summary (advisory)              | Velocity   | Silence Target |
| ----- | ------------------- | ---------------------------------- | ---------- | -------------- |
...
> **Direction:** Open with the audience's pain. Build recognition. Fast pacing —
> the viewer should feel the frustration before we offer relief.

### Beat 1.1 — Hook [target: 5s]

<narration>[warmly] How long does it take to plan a new project at your company? A day? A week of back-and-forth emails?</narration>
...
<narration>Watch this.</narration>

---

## Scene 2: Three Fields and Done [target: 25s]

> **Direction:** Relief phase. Show the solution working. Flowing rhythm —
> connect the three form fields into a single breath.
...
<narration>Name it, deadline, team lead — done.</narration>
...
> **Direction:** This is the wow. Deliberate pacing. Long silence before and
> after the reveal. Let the product speak for itself.

### Beat 3.1 — Setup [target: 4s]

<narration>[confidently] Now here's the part that changes everything.</narration>
...
<narration>[impressed] Ten tasks. Time estimates based on your team's actual history. [pause: 0.5s] Five seconds.</narration>
...
> **Direction:** Quick, confident close. Quantify the value.
...
<narration>[warmly] From nothing to a fully planned project. Ninety seconds. Try it yourself.</narration>
```

**Format rules:**

1. `<narration>` tags wrap all spoken text — TTS reads nothing else
2. `*(silence — Ns. Description.)*` for explicit silent beats with target duration
3. Direction blocks (`> **Direction:**`) are advisory — not passed to TTS
4. Beat headings include `[target: Ns]` for target duration
5. Scene headings use `## Scene N: Title [target: Xs]`
6. Mark the hero scene with `⭐ HERO`
7. Beat map at top establishes velocity and silence targets per scene
8. **No Playwright code, no selectors, no marker IDs** — those live in script.md

### audio/timestamps.json Format

Word-level and phrase-level timestamps extracted from TTS output via Whisper.
This is the primary timing target for the Playwright spec.
...
```json
{
  "scenes": [
    {
      "sceneId": "scene-1",
      "audioFile": "audio/scene-1.wav",
      "durationMs": 7200,
      "words": [
        { "word": "How", "startMs": 0, "endMs": 180 },
        { "word": "long", "startMs": 200, "endMs": 450 },
        { "word": "does", "startMs": 470, "endMs": 620 }
      ],
      "phrases": [
        {
          "text": "How long does it take to plan a new project at your company?",
          "startMs": 0,
          "endMs": 3200,
          "beatId": "1.1"
        },
        {
          "text": "A day? A week of back-and-forth emails?",
          "startMs": 3400,
          "endMs": 5800,
          "beatId": "1.1"
        }
...
- `sceneId` — matches scene heading ID (e.g., "scene-1")
- `durationMs` — total audio duration for this scene
- `words[].startMs`, `words[].endMs` — word-level boundaries (from Whisper)
- `phrases[].beatId` — links to narration.md beat (e.g., "1.1", "2.2-2.3")
- `phrases[].startMs`, `phrases[].endMs` — phrase-level boundaries

### script.md Format — Action Map

The script is an **action map only** — it maps UI actions to audio timestamps
from narration.md. It contains **no narration text** (that lives in narration.md).
Each action references a phrase from narration.md by beat ID and audio timestamp.

#### Scene Heading Format

```markdown
## Scene N: Title [audio: Xs narration + Ys silence + Zs pad = Ts total]
```

The audio budget is derived from TTS output durations and narration.md silence beats.

#### Beat Heading Format

```
### {scene}.{beat} — {label}  → `{marker-id}`
**Phrase Anchor:** "{quoted narration text}" @ {audio-ms}ms
```
...
A markdown table with `At (audio ms)`, `Playwright Code`, and `Action` columns:

```markdown
| At (audio ms) | Playwright Code                        | Action                       |
| ------------- | -------------------------------------- | ---------------------------- |
| 0             | `mark("scene-2", "start")`             | Scene start                  |
| 2400          | `await moveTo(page, newBtn, prevBox)`  | Move to New Project button   |
| 2600          | `await newBtn.click()`                 | Click New Project            |
| 5000          | `await page.waitForTimeout(1500)`      | Let viewer watch redirect    |
```

**Table rules:**

1. Each row maps an audio timestamp to a Playwright action.
2. "At (audio ms)" is the absolute offset from scene start — derived from timestamps.json or narration.md beat targets.
3. **Silent beats** — Rows with only a wait/pause action, no phrase anchor.
4. Rows are ordered chronologically — top to bottom is the execution order.
5. Playwright code column is explicit — removes ambiguity about what gets generated.
...
3. Scene headings include the audio budget.
4. **Phrase Anchor** line references the narration text and its audio timestamp.
...
> **Preferred approach:** Copy the shared template (`plugins/demofly/scripts/helpers.ts`)
...
await page
  ...
  .pressSequentially("My Project", { delay: 35 });
...
| "The loading spinner indicates processing."  | _(silence — let the viewer watch it work)_                                      |
...
1. **Mirror narration** — Describing exactly what the viewer can see.
   ...
   - ✅ _(silence during click)_ → "Account created. No email verification, no waiting."
...
4. **Captain Obvious** — Narrating things the viewer figured out 2 seconds ago.
   ...
   - ✅ _(skip — move to the next meaningful action)_
...
6. **Permission narration** — Asking the viewer's permission to proceed.
   ...
   - ✅ "Settings." _(navigate)_ "Three toggles. No docs required."
...
- [ ] **Problem vividness test:** Problem section makes the viewer _feel_ frustration?
...
- **Sentence structure:** Flowing narrative sentences that build on each other and tell a story. Compound sentences are encouraged — use them to connect cause and effect, pain and relief, the old way and the new way. Each sentence should make the next one feel inevitable. Never choppy fragments or staccato bursts. A single narration beat should read like a paragraph from a keynote speaker who's about to change how you think about this problem.
- **Vocabulary:** Zero jargon. No acronyms unless universally known (AI, URL). "You/your" pronoun. Use evocative, sensory language that makes the viewer *feel* the problem and *crave* the solution. Paint a picture of the world before and after. Words like "imagine," "finally," "because," "that means" — connective tissue that pulls the viewer through the story.
- **TTS tags:** `[warmly]` for intro/outro, `[confidently]` for features, `[impressed]` for hero. Light pauses between story beats, not between fragments.
- **Silence ratio:** 35% — generous breathing room. Silence is where the product speaks for itself. Let the viewer watch and absorb.
- **Cursor speed:** Slow/smooth (40 steps in `moveCursor`). Deliberate, executive feel.
- **Lead-in:** 800ms — narrator speaks well before action begins. The voice sets up the story, and the product delivers the proof.
- **Story-driven tone:** The narration should hook the viewer by naming a pain they recognize, build anticipation for the solution, and then let the product deliver the payoff. Every sentence should answer "why should I care?" before the viewer has to ask it.
- **Example tone:** "You know that feeling — you've spent the last twenty minutes wrestling with something that should have taken two, and you're starting to wonder if there's a better way. There is, and you're about to see exactly why it changes everything." / "Think about every time you've had to start from scratch because the tool couldn't keep up with your thinking — that friction disappears the moment you see what happens next."
...
### In script.md (Action Map)

Account for lead-in when calculating phrase anchor timestamps. The action's
"At (audio ms)" should be `phrase_end_ms + lead_in_ms`:

```markdown
### 2.1 — Create Project → `scene-2:click:create-btn`
**Phrase Anchor:** "With our project in place..." @ 2400ms
```

The Playwright action fires at 2400ms + 800ms lead-in = 3200ms into the scene.

---

## 12. Complete Example (Annotated)

This example demonstrates the narration-first pipeline with two artifacts:
narration.md (storytelling) and script.md (action map).

### narration.md Example

```markdown
# Narration: TaskFlow Demo
...
> **Direction:** Open with pain. Fast pacing. Viewer should feel frustration.

### Beat 1.1 — Hook [target: 6s]
...
> **Direction:** Relief. Flowing rhythm — three fields feel like one breath.
...
> **Direction:** Quick, confident close.
...
### Corresponding script.md (Action Map) Example

```markdown
# Action Map: TaskFlow Demo

**Product:** TaskFlow
**Audio source:** narration.md → audio/ (TTS generated)
**Scenes:** 4
...
| At (audio ms) | Playwright Code                                      | Action                 |
| ------------- | ---------------------------------------------------- | ---------------------- |
| 0             | `mark("scene-1", "start")`                           | Scene start            |
| 0             | `await page.goto(url)`                               | Navigate to dashboard  |
| 800           | `await page.waitForTimeout(800)`                     | Lead-in gap            |

### 1.2 — Dashboard Glance → `scene-1:hover:active-projects`

| At (audio ms) | Playwright Code                                      | Action                     |
| ------------- | ---------------------------------------------------- | -------------------------- |
| 6200          | `prevBox = await moveTo(page, activeProjects, prev)` | Hover Active Projects card |
| 8200          | `prevBox = await moveTo(page, teamPanel, prevBox)`   | Hover Team Availability    |

### 1.3 — Transition → `scene-1:click:new-project-btn`
**Phrase Anchor:** "Watch this." @ 10200ms

| At (audio ms) | Playwright Code                                      | Action                    |
| ------------- | ---------------------------------------------------- | ------------------------- |
| 10200         | `prevBox = await moveTo(page, newBtn, prevBox)`      | Move to New Project       |
| 11000         | `await newBtn.click()`                               | Click New Project         |
| 11000         | `mark("scene-1", "end")`                             | Scene end                 |
...
| At (audio ms) | Playwright Code                                      | Action                  |
| ------------- | ---------------------------------------------------- | ----------------------- |
| 0             | `mark("scene-3", "start")`                           | Scene start             |
| 800           | `prevBox = await moveTo(page, suggestBtn, prevBox)`  | Move to AI Suggest      |
| 2000          | `await suggestBtn.click()`                           | Click AI Suggest Tasks  |

### 3.2 — The Reveal → `scene-3:wait-start:ai-suggestions`

| At (audio ms) | Playwright Code                                      | Action                  |
| ------------- | ---------------------------------------------------- | ----------------------- |
| 4000          | `mark("scene-3", "wait-start", "ai-suggestions")`   | Begin wait              |
| 4000          | `await page.getByText("Generating...").waitFor()`    | Wait for spinner        |
| 12000         | `mark("scene-3", "wait-end", "ai-suggestions")`     | Suggestions appear      |
```

**What makes this work:**

- narration.md is pure storytelling — no selectors, no Playwright code
- script.md is pure action map — no narration text, only phrase anchors
- Audio timestamps in "At (audio ms)" drive Playwright pacing via `waitUntil()`
- Direction blocks in narration.md guide the narrator, not the code generator
- Silent beats have explicit durations — no guessing
- Hero scene (⭐) gets 40% silence target and deliberate velocity

---

## 13. TTS, Timestamps, and Intelligent Assembly

### Narrate First, Record Second

Narration is the source of truth. Write the story, generate audio, then record
video that serves the narration. Intelligent assembly reconciles timing drift.

### Workflow

1. Write narration.md (Step 5 in SKILL.md)
2. Run TTS: `demofly generate <name> --audio` — reads narration.md, produces audio/
3. Optionally extract timestamps: `demofly generate <name> --timestamps` — produces timestamps.json
4. Write script.md (action map referencing audio timestamps)
5. Generate demo.spec.ts with audio-informed timing
6. Record: `npx playwright test` — produces video.webm + timing.json
7. Assemble: `demofly generate <name>` (simple) or `demofly generate <name> --assemble` (intelligent)
...
- If the beat's target is much longer than read time, add pauses rather than words.

### TTS + Assembly via CLI

The `demofly generate` command handles TTS audio generation and video assembly.

**TTS-only** (for iterating on narration):

```bash
demofly generate <name> --audio
```

**Single-scene TTS** (regenerate one scene):

```bash
demofly generate <name> --audio --scene scene-2
```

**Timestamp extraction** (Whisper word-level timestamps):

```bash
demofly generate <name> --timestamps
```

**Simple assembly** (adelay + amix stitching):

```bash
demofly generate <name>
```

**Intelligent assembly** (alignment + LLM edit proposals + ffmpeg retiming):
...
- `--voice <name>` — TTS voice name (default: `heart` / Kokoro `af_heart`).
- `--provider <provider>` — TTS provider: `kokoro` (local, free), `elevenlabs`, or `openai` (cloud, subscription required).
- `--speed <multiplier>` — Speech rate (default: `1.0`).
- `--audio` — Run TTS only, skip recording and assembly.
- `--video` — Run recording only, skip TTS and assembly.
- `--timestamps` — Run Whisper on audio files, produce timestamps.json.
- `--align` — Produce alignment.json from timestamps.json + timing.json.
- `--assemble` — Full intelligent assembly (align → LLM edit proposals → ffmpeg retiming → stitch).
- `--scene <id>` — Target a specific scene for audio regeneration.
- `--no-audio` — Skip TTS entirely, assemble video without narration.

#### Voices

**Kokoro (local, free — no auth required):**
`af_heart` (default), `af_bella`, `af_nicole`, `af_nova`, `af_sarah`, `am_adam`, `am_michael`.

**Cloud voices (subscription required):**
ElevenLabs and OpenAI voices are available via `--provider`. Use `demofly voices list` to see available voices and `demofly voices select` to set a default. If cloud TTS is unavailable (network error, no subscription), the CLI falls back to Kokoro automatically.
...
- `demofly/<name>/narration.md` (for TTS; falls back to `transcript.md` for legacy demos)
- `demofly/<name>/recordings/video.webm` (for assembly)
- `demofly/<name>/recordings/timing.json` (for assembly)
- `demofly/<name>/audio/timestamps.json` (for intelligent assembly, optional)

Produces:

- `demofly/<name>/audio/*.wav` or `*.mp3` (per-scene audio)
- `demofly/<name>/audio/timestamps.json` (with `--timestamps`)
- `demofly/<name>/recordings/alignment.json` (with `--align` or `--assemble`)
- `demofly/<name>/recordings/edit-decisions.json` (with `--assemble`)
- `demofly/<name>/recordings/final.mp4`

The agent does NOT run ffmpeg directly — `demofly generate` handles all assembly.

### Alignment Format (alignment.json)

Compares audio timestamps against video DEMOFLY markers per beat:
  ...
  "beats": [
    {
      ...
      "syncPoints": [
        {
          "phraseText": "How long does it take...",
          "audioMs": 0,
          "videoMarker": "scene-1:start",
          "videoMs": 0
        }
      ]
    }
...
- `driftMs` — positive = video longer, negative = audio longer
- `status` — `"aligned"` (|drift| < 500ms), `"needs_adjustment"` (500-2000ms), `"critical"` (> 2000ms)
- `syncPoints` — high-confidence anchors linking phrases to markers

### Edit Decisions Format (edit-decisions.json)

LLM-generated retiming instructions, auto-applied:
  ...
  "decisions": [
    {
      ...
      "rationale": "Video slightly longer than narration; minor speedup keeps sync",
      "confidence": 0.85
    },
    {
      "beatId": "3.2",
      ...
      "rationale": "Hero reveal needs extra dwell time for AI generation visual",
      "confidence": 0.92
    }
  ]
}
...
**Edit types:** `speed_video`, `slow_video`, `freeze_frame`, `trim_silence`,
`insert_crossfade`, `pad_silence`, `no_change`
...
The helper functions (`createMarker`, `moveTo`, `waitUntil`, `injectCursor`) are
maintained in a single template file:

```
plugins/demofly/scripts/helpers.ts
```
...
| **Final**     | `demofly/<name>/`           | proposal.md, narration.md, script.md, demo.spec.ts, recordings/, audio/ | Permanent              |
...
```bash
# Transient
TMPDIR=$(node plugins/demofly/scripts/create-temp-dir.js my-demo --type transient)

# Session
SESSDIR=$(node plugins/demofly/scripts/create-temp-dir.js my-demo --type session)
```
...
| proposal.md, narration.md, script.md | Final     | `demofly/<name>/`                     |
| demo.spec.ts, playwright.config.ts   | Final     | `demofly/<name>/`                     |
| TTS audio files, timestamps.json     | Final     | `demofly/<name>/audio/`               |
| timing.json, video.webm, final.mp4   | Final     | `demofly/<name>/recordings/`          |
| alignment.json, edit-decisions.json   | Final     | `demofly/<name>/recordings/`          |
